...
impl LoginInfoScreen {
    pub fn draw_window(cursive: &mut cursive::Cursive, login: &Login) {
        let key = cursive
            .user_data::<UserConfig>()
            .unwrap()
            .master_password
            .to_owned();
        let decrypted_password = decrypt(&login.password, &key);
        // todo: figure out ways to prevent cloning.
        let login1 = login.clone();
        let login2 = login.clone();

        let dialog_view = construct_dialog(
            "Info",
            ListView::new()
                .child(
                    "Name:",
                    NamedView::new("login_name", EditView::new().content(&login.name)),
                )
                .child(
                    "Username:",
                    NamedView::new("login_uname", EditView::new().content(&login.username)),
                )
                .child(
                    "Password:",
                    NamedView::new(
                        "login_password",
                        EditView::new().content(&decrypted_password),
                    ),
                ),
        )
        .button("Copy to clipboard", move |x| {
            // messy code!
            let mut clipboard: ClipboardContext = ClipboardProvider::new().unwrap();
            if clipboard
                .set_contents(String::from(&decrypted_password))
                .is_ok()
            {
        ...
        .button("Delete", move |x| {
            ...
            x.add_layer(
                construct_dialog(
                    ...
                    TextView::new("Press ok to go back"),
                )
                .button("Ok", |y| {
                    y.pop_layer();
                    y.pop_layer();
        ...
        .button("Save changes", move |x| {
            let name = x
                .find_name::<EditView>("login_name")
                .unwrap()
                .get_content()
                .to_string();
            let uname = x
                .find_name::<EditView>("login_uname")
                .unwrap()
                .get_content()
                .to_string();
            let password = x
                .find_name::<EditView>("login_password")
                .unwrap()
                .get_content()
                .to_string();
            ...
            x.add_layer(
                construct_dialog(
                    "Operation completed.",
                    TextView::new("Changes saved successfully!"),
                )
                .button("Ok", |cx| {
                    cx.pop_layer();
                    cx.pop_layer();
